function PortalArticleDocTabs(id) {
  this.id      = id;
  this.tabs    = null;
  this.content = null;
  this.links   = null;
  ...
  this.show = function(name) {
    ...
    // select list
    var list = this.content.getElementsByTagName('div');
    for (var i = 0; i < list.length; i++) {
      if (!list[i].className.match(new RegExp('(^| )tabDocs( |$)'))) {
        continue;
      }
      if (list[i].className.match(new RegExp('(^| )' + name + '( |$)'))) {

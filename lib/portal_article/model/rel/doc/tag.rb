module PortalArticle::Model::Rel::Doc::Tag
  attr_accessor :in_tags
  
  def self.included(mod)
    mod.has_many :tags, :primary_key => 'unid', :foreign_key => 'unid', :class_name => 'PortalArticle::Tag',
  ...
  def save_tags
    ...
    3.times do |i|
      if value = values[i]
        unless  tag = tags[i]
          tag = PortalArticle::Tag.new({:unid => unid, :name => i})
        end
        tag.word = value
        tag.save
      elsif tag = tags[i]

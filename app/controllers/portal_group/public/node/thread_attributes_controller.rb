# encoding: utf-8
class PortalGroup::Public::Node::ThreadAttributesController < Cms::Controller::Public::Base
  def pre_dispatch
  ...
  def index
    @items = PortalGroup::Attribute.root_items(:content_id => @content.id, :state => 'public')
  end

  def show
    thread = PublicBbs::Thread.new.public
    thread.and :portal_group_id, @content.id
    thread.portal_attribute_is PortalGroup::Attribute.find(@item.id)

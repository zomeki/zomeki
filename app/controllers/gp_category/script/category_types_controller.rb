...
class GpCategory::Script::CategoryTypesController < Cms::Controller::Script::Publication
  def publish
    ...
    publish_more(@node, :uri => uri, :path => path, :limit => 1, :dependent => :more)
    ...
    category_types.each do |category_type|
      ...
      publish_more(category_type, :uri => uri, :path => path, :limit => 1, :dependent => "#{category_type.name}/more")
      publish_more(category_type, :uri => uri, :path => path, :limit => 1, :file => 'more', :dependent => "#{category_type.name}/more_docs")
  ...
  def publish_category(cat, follow_children: true)
    ...
    publish_more(cat.category_type, :uri => uri, :path => path, :limit => 1, :dependent => "#{cat_path}more")
    publish_more(cat.category_type, :uri => uri, :path => smart_phone_path, :limit => 1, :dependent => "#{cat_path}more_smart_phone", :smart_phone => true)
    publish_more(cat.category_type, :uri => uri, :path => path, :limit => 1, :file => 'more', :dependent => "#{cat_path}more_docs")
    publish_more(cat.category_type, :uri => uri, :path => smart_phone_path, :limit => 1, :file => 'more', :dependent => "#{cat_path}more_docs_smart_phone", :smart_phone => true)

  ...
  def index
    ...
    GpArticle::Piece::RecentTabXml.find(:all, @piece, :order => :sort_no).each do |tab|
      ...
      unless tab.categories_with_layer.empty?
        ...
        docs = GpArticle::Doc.public.where(content_id: @piece.content.id).where(id: doc_ids).order('published_at DESC').limit(@piece.list_count)
      else
        docs = GpArticle::Doc.public.where(content_id: @piece.content.id).order('published_at DESC').limit(@piece.list_count)
      end

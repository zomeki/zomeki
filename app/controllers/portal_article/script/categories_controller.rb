# encoding: utf-8
class PortalArticle::Script::CategoriesController < Cms::Controller::Script::Publication
  def publish
    return render(:text => "OK")
    
    units = PortalArticle::Unit.find_departments(:web_state => 'public')
    
    cond = {:state => 'public', :content_id => @node.content_id}
    PortalArticle::Category.root_items(cond).each do |item|

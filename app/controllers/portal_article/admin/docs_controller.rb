# encoding: utf-8
class PortalArticle::Admin::DocsController < Cms::Controller::Admin::Base
  include Sys::Controller::Scaffold::Base
  include Sys::Controller::Scaffold::Recognition
  include Sys::Controller::Scaffold::Publication
  helper PortalArticle::FormHelper
  helper PortalGroup::FormHelper

  def pre_dispatch
    return error_auth unless @content = PortalArticle::Content::Doc.find(params[:content])
    ...
    if @portal_group == nil
      ...
      if !Core.site.portal_group_id.blank?
        setting = PortalArticle::Content::Setting.config(@content, 'portal_group_id')
        setting.value = Core.site.portal_group_id
        setting.save
      else
  ...
  def index
    item = PortalArticle::Doc.new#.public#.readable

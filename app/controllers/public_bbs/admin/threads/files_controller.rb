# encoding: utf-8
class PublicBbs::Admin::Threads::FilesController < Cms::Controller::Admin::Base
  layout 'public/files'
...
item.in_creator = {'group_id' => user.id, 'user_id' => user.groups.first.id}
  ...
  def public_files_uri
    if @tmp_unid
      "#{public_bbs_thread_path(@content, @tmp_unid)}/files"
    else
      "#{public_bbs_thread_path(@content, @thread)}/files"
    end
  end

  def public_file_contents_uri
    if @tmp_unid
      "#{public_bbs_thread_path(@content, @tmp_unid)}/file_contents"
    else
      "#{public_bbs_thread_path(@content, @thread)}/file_contents"
    end
  end
end

...
class PublicBbs::Public::Node::Responses::FilesController < Cms::Controller::Public::Base
  ...
  def pre_dispatch
    @thread = PublicBbs::Thread.find(params[:node_thread_id])
  ...
  def index
    ...
    if @tmp_unid
      ...
      @item.and :parent_unid, @response.unid
    end
...
item.in_creator = {'group_id' => user.id, 'user_id' => user.groups.first.id}
    if @tmp_unid
      ...
      item.parent_unid = @response.unid
    end
  ...
  def public_files_uri
    if @tmp_unid
      "#{@thread.public_responses_uri}/#{@tmp_unid}/files"
    else
      "#{@thread.public_responses_uri}/#{@response.id}/files"
    end
  end

  def public_file_contents_uri
    if @tmp_unid
      "#{@thread.public_responses_uri}/#{@tmp_unid}/file_contents"
    else
      "#{@thread.public_responses_uri}/#{@response.id}/file_contents"
    end
  end
end
